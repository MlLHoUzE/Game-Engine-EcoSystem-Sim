...
int main()
{
	...
	{
		iPlant* plant;
		plant = pPlantBuilder->createPlant("Lettuce");
		...
		plant->Scale(0.1f);
		plant->SetMeshEntry(monkeyMesh.NumIndices, monkeyMesh.BaseIndex, monkeyMesh.BaseVertex);
		plant->SetColor(glm::vec4(1.0, 1.0, 0.0, 1.0));
		//pMediator->addPlant(plant);
		gPlants.push_back(plant);
	}
	...
	success = LoadMesh("assets/MilleniumFalcon_N.ply", indices, vertices, unicornMesh);
	...
	for (int index = 0; index != 80; index++)
	{
		...
		{
			animal = pAnimalBuilder->createAnimal("Unicorn");
			animalMesh = unicornMesh;
			animalScale = 8.f;
		}
	...
	do {
		...
		if (gAnimals.size() == 0)
		{
			std::cout << "All Animals DEAD!" << std::endl;
		}
		DrawScene();
...
void DrawTheFloor()
{
	...
	glm::vec4 boxColorLight(0.133333f, 0.545098f, 0.133333f, 1.0f);

...
cUnicorn::cUnicorn()
{
	...
	bActivate = false;
	bIsDead = false;
	m_prefs.pref1 = gPreferences::PINEAPPLE;
	m_prefs.pref2 = gPreferences::LETTUCE;
}
//cEnemy::~cEnemy() {}


void cUnicorn::GetTransform(glm::mat4& transform)
{
	transform = m_eTransform;
}

void cUnicorn::SetPosition(glm::vec3& position)
{
	m_eTransform = glm::translate(m_eTransform, position);
}

void cUnicorn::GetMeshEntry(sMeshEntry& meshEntry)
{
	meshEntry = m_eMeshEntry;
}

void cUnicorn::SetMeshEntry(int NumIndices, int BaseIndex, int BaseVertex)
{
	m_eMeshEntry.NumIndices = NumIndices;
	m_eMeshEntry.BaseIndex = BaseIndex;
	m_eMeshEntry.BaseVertex = BaseVertex;
}
void cUnicorn::GetColor(glm::vec4& color)
{
	color = m_eColor;
}
void cUnicorn::SetColor(glm::vec4& colorIn)
{
	m_eColor = colorIn;
}
...
void cUnicorn::SetID(int ID)
{
	m_ID = ID;
}

void cUnicorn::Scale(float scale)
{
	m_eTransform = glm::scale(m_eTransform, glm::vec3(scale, scale, scale));
}

void cUnicorn::Update(float dt)
{
	if (secondCounter > 1)
	{
		m_calories -= 125;
...
void cUnicorn::setMediator(iMediator* mediator)
{
	m_pMediator = mediator;
}
int cUnicorn::GetType()
{
	return m_type;
}

float cUnicorn::getCalories()
{
